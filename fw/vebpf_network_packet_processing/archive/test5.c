#include "mmio.h"
//#include<string.h>  // doesnt need that?
...
char uart_output[] = "Hello from Arty FPGA to PC";
...
// 		write_uart(112);
		...
		for(int i = 0; i < (sizeof(uart_output)-1); i++){		// strlen doesnt work. its a new func. so crosscompilers might work due to being old
// 		//for(int i = 0; i < 6; i++){
		// for(int i = 0; i < 5; i++){
