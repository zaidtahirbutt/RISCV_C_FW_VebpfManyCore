// ===========================================================================
// board.h -- peripheral definitions for the DISL / VeBPF many-core RISC-V
// control plane.
//
// PROVENANCE: this file is inherited from the ZipVersa project (Gisselquist
// Technology) and still carries definitions for a great deal of hardware that
// DOES NOT EXIST in this design. That is mostly harmless -- 764 raw lines
// reduce to ~66 after preprocessing, because the ZipVersa autofpga convention
// wraps each peripheral in `#ifdef <NAME>_ACCESS` and only a few of those are
// ever defined. But it is invisible to a reader, so the state is written down
// here.
//
// WHAT THIS BOARD ACTUALLY HAS, per [INSTANTIATIONS] and [INSTANTIATIONS.cpu.MAP]
// in the consuming project's system.tml:
//
//     cache    0x00000000   BRAM holding this firmware   -> MEM
//     gpio     0x10000000   gpio_axi                     -> GPIO
//     debug    0x10000018   uart_axi (the debug UART)    -> UART
//     timer    0x1000001C   timer_axi                    -> TIMER
//     eth_nic  0x20000000   network subsystem + VeBPF    -> ETH_NIC
//
// Those names come from address_map.h, which disl-core GENERATES from that MAP
// (see [ADDRESS_MAP_HEADER] MODE="generate"). It is included below, so the
// hardware address map and this file cannot drift apart.
//
// FEATURE GATES -- which `#ifdef` blocks are on:
//
//     ON   NET1_ACCESS       network subsystem CSRs + packet structs   (real)
//     ON   BUSPIC_ACCESS     interrupt controller                      (real)
//     off  GPIO_ACCESS       ZipVersa GPIO block; this design's GPIO is
//                            reached through the generated GPIO address
//     off  FLASH_ACCESS, FLASHCFG_ACCESS      no flash on this board
//     off  WBFFT_ACCESS                        no FFT accelerator
//     off  SPIO_ACCESS                         no SPIO
//     off  BKRAM_ACCESS                        no separate block RAM peripheral
//     off  BUSCONSOLE_ACCESS                   no ZipCPU bus console
//     off  PWRCOUNT_ACCESS                     no power counter
//     off  NETCTRL1_ACCESS                     alternate net-memory CSR block
//     off  ZIPVERSA_LEGACY_PERIPHERALS         _buserr / _net1dly / ZIPTIMER
//
// Two guards in the original are MISSPELLED and therefore permanently dead:
// `NETSCOPE_SCOPE` and `FLASHSCOPE_SCOPC` (both should end `_SCOPE`). They are
// left as they are on purpose -- correcting the spelling would ENABLE blocks
// referencing debug-scope hardware this design does not have.
//
// The live surface after preprocessing is small: the build flags, ETH_NIC and
// the network pointers derived from it, the NET_*/ENET_* CSR bit definitions,
// the NETWORK_CSR / ENETPACKET / ENETMDIO types, and _buspic.
// ===========================================================================
...
//
// address_map.h is GENERATED by disl-core from that MAP -- see
// [ADDRESS_MAP_HEADER] MODE="generate" in system.tml. Including it here rather
// than restating the base means there is exactly one definition of where the
// network subsystem lives, and moving it in system.tml moves the firmware too.
//
// board.h is included BEFORE riscv_subsystem/sw/utils.h in every app, and
// utils.h pulls in the same header -- hence the include guard in the generated
// file. Reached via -I$(ADDR_MAP_DIR) from the Makefile.
#include "address_map.h"

// Fallback for the case where address_map.h exists but predates ETH_NIC being
// added to it (i.e. MODE was never run as "generate"), or for a build that
// passes -DETH_NIC=0x... explicitly. When the generated header defines it, this
// is skipped and the hardware map wins.
#ifndef ETH_NIC
...
// ZipVersa-era registers at ZipCPU addresses (0x006xxxxx). NO SUCH HARDWARE
// EXISTS in this design -- see the peripheral table at the top of this file --
// so they are gated off. They were previously ungated and unused, which meant
// they survived preprocessing and looked live. Using one would have poked an
// address the interconnect does not decode.
#ifdef	ZIPVERSA_LEGACY_PERIPHERALS
#define	_BOARD_HAS_BUSERR
static volatile unsigned *const _buserr = ((unsigned *)6291460);	// 0x00600004
#define	_BOARD_HAS_NET1DLY
static volatile unsigned *const _net1dly = ((unsigned *)0x0060001c);
#endif	// ZIPVERSA_LEGACY_PERIPHERALS
#ifdef	WBFFT_ACCESS
...
// ZIPTIMER: the ZipVersa system timer at 0x00600028. This design's timer is a
// different peripheral at a different address -- TIMER (0x1000001C) from the
// generated address map. _systimer is unused, and pointing it at the ZipCPU
// address would read a location the interconnect does not decode, so the block
// is gated off. Use TIMER if you need the timer.
#ifdef	ZIPVERSA_LEGACY_PERIPHERALS
