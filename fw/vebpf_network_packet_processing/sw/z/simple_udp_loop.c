...
// ************** I have hard coded the dest mac address so I dont have to use ARP ************************* ////////////////
	...
	/// our transmission packet for udp tx pkt
	NET_PACKET *tx_udp_pkt;

	unsigned	host_ip = DEFAULT_ROUTERIP;  // IPADDR(192,168,0,1)  // in etcnet.h #define	IPADDR(A,B,C,D)	((((A)&0x0ff)<<24)|(((B)&0x0ff)<<16)|(((C)&0x0ff)<<8)|(D&0x0ff))
	...
	// call atomic store
	__atomic_store_n((unsigned*)&(_net1->n_txcmd), 0, memorder);  // here the txcmd bit is set to 0 here (the data here on this address _net1->n_txcmd)



	// tx_cmd_test = _net1->n_txcmd;
	tx_cmd_test = __atomic_load_n((unsigned*)&(_net1->n_txcmd), memorder);  // just testing by reading the _net1->n_txcmd value
	...
	*/
	...
	// transmitting random payload so it can be checked in wireshark
	tx_udp_pkt->p_user[0] = (5) & 0xff;
	tx_udp_pkt->p_user[1] = (9) & 0xff;
	tx_udp_pkt->p_user[2] = (50) & 0xff;
	tx_udp_pkt->p_user[3] = (90) & 0xff;

	// transmit the udp packet
	//tx_udp(NET_PACKET *pkt, unsigned dest, unsigned sport, unsigned dport) {
	tx_udp(tx_udp_pkt, host_ip,UDP_DEV_PORT, UDP_HOST_PORT);

	// UART printf for our device (FPGA)
	printf("First UDP packet has been sent \n");
	...
	while(1){
		...
		if (waiting_pkt == NULL) {  // make sure we don't have a packet waiting in line to be sent due to the hw tx pipeline being busy (FPGA ethernet module)
			...
			// UART printf for our device (FPGA)
			printf("UDP packet # %d has been sent \n", packet_counter);
		}
