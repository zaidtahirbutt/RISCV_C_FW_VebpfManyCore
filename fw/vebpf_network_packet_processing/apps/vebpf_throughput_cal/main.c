...
int	main(int argc, char **argv) {
	...
	while(1) {
	    ...
	    if ((rxpkt_vebpf_dest != EBPF_RESULT_DONT_CARE) && (rxpkt_vebpf_error == 0)) {
	    	...
	    	// 	printf("\n\nled = 7 and rxpkt VeBPF CPU eBPF filtering result = %d and count_malicious_rxpkts = %d \n\n", rxpkt_vebpf_dest, count_malicious_rxpkts); 
	    	// }

	    	// rx_pkt_1 = rx_pkt_v6(&rxpkt_vebpf_dest, &rxpkt_vebpf_valid, &rxpkt_vebpf_error, NOT_LOADING_RX_PKT_DATA);
	    	rx_pkt_v6_reduced();
		    ...
		    // 	if ((!SIMULATION_TESTING) && DEBUG)
	    	// 		printf("Before function free_pkt() rx_pkt_1->p_usage_count = %d \n\n", rx_pkt_1->p_usage_count);
		    ...
		    // 		if ((!SIMULATION_TESTING) && DEBUG)
		    // 			printf("Value of rx_pkt_1->p_usage_count after 1st function of free_pkt() of rx_pkt_1->p_usage_count = %d \n\n", rx_pkt_1->p_usage_count);
		...
		// printf("\n\nTotal count_malicious_rxpkts = %d \n\n", rxpkt_vebpf_dest, count_malicious_rxpkts); 

		// if (count_malicious_rxpkts >= TOTAL_RXPKTS_IN_EXPERIMENT_OF_FIREWALL_TYPE1) {
		// if (count_malicious_rxpkts >= TOTAL_RXPKTS_IN_EXPERIMENT_OF_FIREWALL_TYPE4) {

		// these constants changed
			// #define IGNORE_INITIAL_FIREWALL_MALICIOUS_RXPKTS 4
			// #define TOTAL_MALICIOUS_RXPKTS_FOR_LATENCY_CAL 5
