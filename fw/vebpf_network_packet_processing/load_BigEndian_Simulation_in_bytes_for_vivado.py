# this script converts RISCV pgm_mem hex file to a sim hex file for the VIVADO simulator to simulate the C code for softcore RISCV for FPGA simulated in the VIVADO simulator
...
# name of output sim file for vivado
with open('firmware_VIVADO_sim.hex', 'w') as f:  
    for word in program:
        # print("word[0] = ", word[0].hex())  # AttributeError: 'str' object has no attribute 'hex'
        # print("word[0] = ", word[0])  # 1
        # print("word[7] = ", word[7])  # 7
        # print("word[6] = ", word[6])  # 3
        
        # word = ['10000137']
        # since 37 is the first LSB of the pgm mem, so for vivado sim, the first byte we write will
        # be the LSB of the pgm mem word.. hence we will do the following below..
        ...
        # print("type of variable word[6] = ", type(word[6]))
        # sys.exit("exited")

        # f.write((word[6]&127).to_bytes(1, byteorder='big'))
        # sys.exit("exited")
